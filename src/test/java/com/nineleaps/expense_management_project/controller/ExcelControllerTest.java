...
class ExcelControllerTest {
    ...
    @Test
    void testGenerateExcelReportForCategoryBreakup_NoData() throws Exception {
        LocalDate startDate = LocalDate.now().minusDays(7);
        LocalDate endDate = LocalDate.now();

        when(excelService.generateExcelAndSendEmail(response, startDate, endDate)).thenReturn(NO_DATA_RESPONSE);

        ResponseEntity<String> responseEntity = excelController.generateExcelReport(response, startDate, endDate);

        assertEquals(200, responseEntity.getStatusCodeValue());
        assertEquals(NO_DATA_RESPONSE, responseEntity.getBody());
    }
    ...
    @Test
    void testGenerateExcelReportForAllReports_NoData() throws Exception {
        LocalDate startDate = LocalDate.now().minusDays(7);
        LocalDate endDate = LocalDate.now();
        StatusExcel status = StatusExcel.PENDING;

        when(excelServiceReports.generateExcelAndSendEmail(response, startDate, endDate, status)).thenReturn(NO_DATA_RESPONSE);
    ...
    @Test
    void testReimburseAndGenerateExcel_NoData() throws Exception {
        when(excelServiceReports.reimburseAndGenerateExcel(response)).thenReturn(NO_DATA_RESPONSE);
