					...
					for (Expense expense : expenseList) {
						if (Boolean.TRUE.equals(expense.getIsReported())) {
							...
							if (yearlyReimbursementRatioMap.containsKey(year)) {
								...
								int totalNumberOfExpenses = (int) (yearlyReimbursementRatioMap.get(year + Constants.CONSTANT2) + 1);
								...
								yearlyReimbursementRatioMap.put(year + Constants.CONSTANT2, (double) totalNumberOfExpenses);
								yearlyReimbursementRatioMap.put(year + Constants.CONSTANT1, reimbursementRatio);
							} else {
								yearlyReimbursementRatioMap.put(year, expense.getAmountApproved());
								yearlyReimbursementRatioMap.put(year + Constants.CONSTANT2, 1.0);
								yearlyReimbursementRatioMap.put(year + Constants.CONSTANT1, expense.getAmountApproved());
							}
						}
					}
				}
						...
						if (Boolean.TRUE.equals(expense.getIsReported()) && expense.getDate().getYear() == year) {
							...
							if (yearlyReimbursementRatioMap.containsKey(monthKey)) {
								Double totalReimbursedAmount =  yearlyReimbursementRatioMap.get(monthKey)
										+ expense.getAmountApproved();
								int totalNumberOfExpenses = (int) (yearlyReimbursementRatioMap.get(monthKey + Constants.CONSTANT2) + 1);
								Double reimbursementRatio = totalReimbursedAmount / totalNumberOfExpenses;

								yearlyReimbursementRatioMap.put(monthKey, totalReimbursedAmount);
								yearlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT2, (double) totalNumberOfExpenses);
								yearlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT1, reimbursementRatio);
							} else {
								yearlyReimbursementRatioMap.put(monthKey, expense.getAmountApproved());
								yearlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT2, 1.0);
								yearlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT1, expense.getAmountApproved());
							}
						}
					}
				}
			...
			public HashMap<String, Double> getYearlyReimbursementRatioForAllCategories() {
				...
				for (String yearKey : yearlyReimbursementRatioMap.keySet()) {
					if (!yearKey.contains("_")) {
						String year = yearKey;
						Double totalReimbursedAmount = yearlyReimbursementRatioMap.get(year);
						Double totalNumberOfExpenses = (yearlyReimbursementRatioMap.get(year + Constants.CONSTANT2));
						double reimbursementRatio = totalReimbursedAmount / totalNumberOfExpenses;
						yearlyReimbursementRatioMap.put(year + Constants.CONSTANT1, reimbursementRatio);
					}
				}
					...
					if (expenseYear == year) {
						...
						if (monthlyReimbursementRatioMap.containsKey(monthKey)) {
							Double totalReimbursedAmount = monthlyReimbursementRatioMap.get(monthKey)
									+ expense.getAmountApproved();
							double totalNumberOfExpenses = (monthlyReimbursementRatioMap.get(monthKey + Constants.CONSTANT2) + 1);
							Double reimbursementRatio = totalReimbursedAmount / totalNumberOfExpenses;

							monthlyReimbursementRatioMap.put(monthKey, totalReimbursedAmount);
							monthlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT2,totalNumberOfExpenses);
							monthlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT1, reimbursementRatio);
						} else {
							monthlyReimbursementRatioMap.put(monthKey, expense.getAmountApproved());
							monthlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT2, 1.0);
							monthlyReimbursementRatioMap.put(monthKey + Constants.CONSTANT1, expense.getAmountApproved());
						}
					}
				}
